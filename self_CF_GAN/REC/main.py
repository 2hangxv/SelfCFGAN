...
from REC.utils.sampler import Sampler
from REC.utils.parser import parse_args
from REC.utils.splitter import split_test
from REC.utils.data import PointData, PairData, UAEData
from REC.utils.loader import load_rate, get_ur, convert_npy_mat, build_candidates_set
from REC.utils.metrics import precision_at_k, recall_at_k, map_at_k, hr_at_k, ndcg_at_k, mrr_at_k
...
if __name__ == '__main__':
    ...
    # get ground truth
    test_ur = get_ur(test_set)  # 返回一个字-- key：用户ID(int)，value：数据集中与用户有交互记录物品ID的set(int)
    total_train_ur = get_ur(train_set)
    # initial candidate item pool
    item_pool = set(range(item_num))
    candidates_num = args.cand_num  # No. of candidates item for predict
    ...
    sampler = Sampler(
        user_num, 
        item_num, 
        num_ng=args.num_ng,  # negative sampling number
    ...
    if args.problem_type == 'point':
        if args.algo_name == 'mf':
            from REC.model.point.MFRecommender import PointMF
            model = PointMF(
                user_num, 
                item_num, 
            ...
            from REC.model.point.FMRecommender import PointFM
            model = PointFM(
                user_num, 
                item_num,
                factors=args.factors,
            ...
            from REC.model.point.NeuMFRecommender import PointNeuMF
            model = PointNeuMF(
                user_num,
                item_num,
                factors=args.factors,
            ...
            from REC.model.point.NFMRecommender import PointNFM
            model = PointNFM(
                user_num,
                item_num,
                factors=args.factors,
            ...
            from REC.model.CDAERecommender import CDAE
            model = CDAE(
            ...
            from REC.model.VAERecommender import VAE
            model = VAE(
                rating_mat=training_mat,
                q=args.dropout,
                epochs=args.epochs,
        ...
        if args.algo_name == 'mf':
            from REC.model.pair.MFRecommender import PairMF
            model = PairMF(
                user_num, 
                item_num,
                factors=args.factors,
            ...
            from REC.model.pair.FMRecommender import PairFM
            model = PairFM(
                user_num, 
                item_num,
                factors=args.factors,
            ...
            from REC.model.pair.NeuMFRecommender import PairNeuMF
            model = PairNeuMF(
                user_num,
                item_num,
                factors=args.factors,
            ...
            from REC.model.pair.NFMRecommender import PairNFM
            model = PairNFM(
                user_num, 
                item_num,
                factors=args.factors,

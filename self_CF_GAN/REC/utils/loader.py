...
def load_rate(src='ml-100k', prepro='origin', binary=True, pos_threshold=None, level='ui'):
    ...
    if src == 'test_data':
        df = pd.read_csv(f'./data/{src}/raw.data', sep='\t', header=None,
                         names=['user', 'item', 'rating', 'timestamp'], engine='python')
    elif src == 'ml-100k':
        df = pd.read_csv(f'./GAN/fun_0.6_sat_0.4/data_df.csv', sep=',', header=None,
                         names=['user', 'item', 'rating', 'timestamp'], engine='python')
        # df = pd.read_csv(f'./GAN/3-0.5-0.5/data_df6.csv', sep=',', header=None,
        #                  names=['user', 'item', 'rating', 'timestamp'], engine='python')
    ...
    if prepro == 'origin':
        ...
        # 预处理方法 e.g."10filter":过滤掉交互记录小于10次的user或item
    ...
    # print(df)
    # print("-" * 30)
    # print("用户数量为：%d" % (user_num))
    # print("-" * 30)
    # print("物品数量为：%d" % (item_num))

...
class Sampler(object):
    ...
    def transform(self, sampled_df, is_training=True):
        ...
        # 创建一个维度为 user_num * item_num 的稀疏矩阵 pair_pos
        # pair_pos 只记录有交互的位置对
        pair_pos = sp.dok_matrix((user_num, item_num), dtype=np.float32)

        for _, row in sampled_df.iterrows():
            pair_pos[int(row['user']), int(row['item'])] = 1.0  # 还是隐式推荐，有交互的置1，没交互的仍为0

        neg_sample_pool = list(range(item_num))  # 初始化负采样池
        # 对Dataframe的'item'一列中每个值(item)出现的次数进行降序排序，生成一个list
        ...
        # print("uni_num: %d" % uni_num)  # uni_num:1
        ex_num = self.num_ng - uni_num
        # print("ex_num:%d" % ex_num)  # ex_num:0
        ...
        for _, row in sampled_df.iterrows():
            ...
            js = []
            # 找uni_num个和当前user u 没有交互的item j 放在js中

...
def split_test(df, test_method='fo', test_size=.2):
    ...
    if test_method == 'ufo':
        ...
        # 按时间戳大小顺序划分数据集
        # df = df.sample(frac=1)
        df = df.sort_values(['timestamp']).reset_index(drop=True) # 按时间戳大小排序
        split_idx = int(np.ceil(len(df) * (1 - test_size)))
        train_set, test_set = df.iloc[:split_idx, :].copy(), df.iloc[split_idx:, :].copy()  # 按照test_size(0.2)划分训练集和测试集
    ...
    # 删除test_set中新添加的0项并加入到train_set中，因为那并不是ground truth
    for index, row in test_set.iterrows():
        if row['rating'] == 0:
            test_set.drop(index=index, inplace=True)
            train_set.append(row, ignore_index=True)
